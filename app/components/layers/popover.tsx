...
import getLayerConfigs from "app/wrappedFeatureCollections/queries/getLayerConfigs";
...
function CopyLayer({ onDone }: { onDone?: () => void }) {
  const rep = usePersistence();
  const transact = rep.useTransact();
  const layerConfigs = useAtomValue(layerConfigAtom);
  const items = [...layerConfigs.values()];
  const [configsToCopy] = useQuery(getLayerConfigs, null);

  return (
    <div>
      <LayerFormHeader isEditing={false}>Copy</LayerFormHeader>
      <TextWell>Copy layer configs from other maps.</TextWell>
      {configsToCopy.length ? (
        <div className="max-h-48 overflow-y-auto placemark-scrollmark pt-2">
          <div className="grid gap-y-2">
            {configsToCopy.map((config) => {
              return (
                <E.Button
                  onClick={async () => {
                    await transact({
        ...
        <TextWell>
          No other layers found in other maps that could be copied.
        </TextWell>
      )}
    </div>
  );
